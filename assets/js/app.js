...
export function handleSliderTabOrders() {
    ...
    if (ordersTab !== null && ordersWrapper !== null) {
        ...
        const scrollElementInSwiper = ordersWrapper.querySelectorAll('.theme-tabs_overflow');
        if (scrollElementInSwiper.length > 0) {
            const lockSliderOrdersWrapper = function () {
                sliderOrdersWrapper.lock;
            }
            const unLockSliderOrdersWrapper = function () {
                sliderOrdersWrapper.unlock;
            }
            let swiperCanSwipe = true;
            scrollElementInSwiper.forEach((item) => {
                /*const itemButtons = item.querySelectorAll('.nav-link');
                if (itemButtons.length > 0) {
                    itemButtons.forEach((button) => {
                        button.addEventListener('mousedown', lockSliderOrdersWrapper);
                        button.addEventListener('touchstart', lockSliderOrdersWrapper);

                        button.addEventListener('mouseup', unLockSliderOrdersWrapper);
                        button.addEventListener('touchend', unLockSliderOrdersWrapper);
                    })
                }*/
                item.addEventListener('scroll', () => {
                    if (swiperCanSwipe) {
                        sliderOrdersWrapper.lock;
                        swiperCanSwipe = false;
                    }
                });

                item.addEventListener('scroll', () => {
                    // Mở khóa chuyển slide cho swiper
                    sliderOrdersWrapper.unlock;
                    swiperCanSwipe = true;
                });
            })
