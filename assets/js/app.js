...
/****
 * Tempalte Select Dropdown
 */
export function handleTemplateSelectDropdown() {
    const selectDropdownCall = document.querySelectorAll('.template-select .template-select_call');
    const selectDropdownChoose = document.querySelectorAll('.template-select .template-select_dropdown--choose');
    if (selectDropdownCall.length > 0 && selectDropdownChoose.length > 0) {
        selectDropdownCall.forEach((select) => {
            select.addEventListener('click', () => {

                const currentDropdown = select.closest('.template-select');
                const isCurrentDropdownVisible = currentDropdown.classList.contains('is-show');

                selectDropdownCall.forEach((otherSelect) => {
                    const otherDropdown = otherSelect.closest('.template-select');
                    if (otherDropdown !== currentDropdown) {
                        otherDropdown.classList.remove('is-show');
                    }
                });
        ...
        selectDropdownChoose.forEach((choose) => {
            choose.addEventListener('click', () => {
                const chooseOthers = choose.closest('.template-select').querySelectorAll('.template-select_dropdown--choose');
                chooseOthers.forEach(function (otherItem) {
                    if (otherItem !== choose && otherItem.classList.contains('is-active')) {
                        otherItem.classList.remove('is-active');
                    }
                ...
                choose.classList.add('is-active');
                choose.closest('.template-select').classList.remove('is-show');
                choose.closest('.template-select').querySelector('.template-select_call span').innerHTML = choose.innerHTML;
            })
        });
        ...
        document.addEventListener('click', (event) => {
            ...
            if (!clickedElement.closest('.template-select.is-show')) {
                const allFilterDropdowns = document.querySelectorAll('.template-select.is-show');
